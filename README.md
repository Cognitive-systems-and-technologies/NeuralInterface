...
# Общее описание
NeuralInterface - это модуль мониторинга и визуализации, используемый для отображения работы агентов. 
...
- устройства систем умного дома (роботы-пылесосы и др. устройства).

Данный модуль связан с модулем:

https://github.com/Cognitive-systems-and-technologies/RoboAICore.

Пример работы веб-приложения с модулем RoboAICore, установленным на агента:

https://github.com/Cognitive-systems-and-technologies/NeuralInterface/assets/47759876/15780a6c-fa6e-4170-a1f9-37d5b954e09b

В данном видео продемонстрировано управление агента с помощью модуля NeuralInterface. При нажатии на кнопки передвижения, мобильный робот перемещается в соответствующем направлении.

Для получения подобного результата, как на видео, необходимо установить оба модуля: NeuralInterface, RoboAICore и настроить таким образом, чтобы запросы из веб-приложения отправлялись на агентов, принимались и отсылали ответ.
В противном случае, модуль NeuralInerface будет работать в тестовом режиме, без агентов и при попытке отправить запрос будет выдавать ошибку, которая будет зависеть от устройств, расположенных в локальной сети и их состояний.

Характеристики агентов.

В качестве тестового образца были выбраны: семейство процессоров STM32 серии STM32F4 на базе архитектуры ARM Cortex-M4 модель STM32F429ZI разрядностью 32bit и микрокомпьютер Raspberry pi 3 (model B). 
...
В самом сайте на странице "Информация о проекте" есть подробная инструкция по использованию. Также, она описана ниже.
...
Административная панель Django - это встроенный инструмент, предоставляемый Django для управления данными вашего веб-приложения. Она обеспечивает удобный и мощный интерфейс для работы с моделями и данными в вашей базе данных без необходимости создания собственного пользовательского интерфейса. Это мощный инструмент, который значительно упрощает управление данными в вашем веб-приложении, и она поставляется с Django "из коробки", что делает ее одним из популярных выборов для административного интерфейса.
